...
score = 0.0
game_state = "MENU" # State awal

# --- INISIALISASI ---
# Asumsi kelas Audio ada di file audio.py
try:
    audio = Audio()
    audio.start()
except NameError:
    print("Error: Pastikan kelas Audio didefinisikan di audio.py.")
    sys.exit()

# asset path (Asumsi aset ada di folder 'assets')
...
Nyawa_path = os.path.join(os.getcwd(), 'assets', 'Nyawa.png') 
...
HEART_SIZE = (40, 40)
HEART_IMG = load_and_scale_asset(Nyawa_path, HEART_SIZE)
...
def reset_game():
    ...
    current_health = MAX_HEALTH
    score = 0.0
    manager.sprite_list.clear() # Hapus semua pagar yang ada
    game_state = "PLAYING"
    print("Game Dimulai!")
...
def camera_follow_on_bounce(dx: float):
    ...
    # Hanya mengurangi nyawa jika game sedang berjalan
    if game_state == "PLAYING":
        current_health -= 1 # Kurangi nyawa
        check_game_over()
        
    offset_x += dx
    manager.shift_world(dx)

# Kamera dan Background FPS
...
try:
    while running:
        ...
        for e in pygame.event.get():
            ...
            # Deteksi tombol START (Spacebar) di Menu atau Game Over
        ...
        if game_state == "PLAYING":
            ...
            collided_first_any, collided_second_any, back_draw, front_draw, debug_boxes = manager.update_and_prepare_draw(
                ...
                z_down=nunduk,
                x_down=ramping,
                camera_follow_cb=camera_follow_on_bounce, # Callback untuk bounce/pengurangan nyawa
        ...
        if game_state != "MENU":
            ...
            if os.path.exists(sapi_path):
                player_img = load_and_scale_asset(sapi_path, (PLAYER_W, PLAYER_H))
            ...
            # Kirim HEART_EMPTY_IMG sebagai None
